...
class WeightInfoGroup : public std::vector<boost::shared_ptr<WeightInfo<T> > >{
    ...
    public:
         ...
         */
        bool updateSelectedCurrentWeight(typename WeightInfoGroup::iterator selectedIter,  int selectedTimes = 1);

        bool updateSelectedCurrentWeight(weightinfo_ptr_t selectedWeightInfo, int selectedTimes = 1);
        /*
         * 往权重组中添加新的权重信息
         */
        ...
        void beginSelect();
        // 选择下一个候选
        bool getNext(T& outData);
        // 确认选择器是可用资源, 结束选择, 更新内部权重信息
        // selectedTimes: 该元素实际相当于被选中的次数
        bool endSelect(size_t selectedTimes = 1);
    ...
    private:
        ...
        typename WeightInfoGroup<T>::iterator m_CurSelectedIterator; 
        int m_iNextSelectIdx;
};
...
template<class T>
bool WeightInfoGroup<T>::updateSelectedCurrentWeight(typename WeightInfoGroup<T>::iterator selectedIter, int selectedTimes)
{
    return updateSelectedCurrentWeight(*selectedIter);
}

template<class T>
bool WeightInfoGroup<T>::updateSelectedCurrentWeight(weightinfo_ptr_t selectedWeightInfo, int selectedTimes)
{
    ...
    // 当作多次处理， 则totalWeight要加，每个元素的currentWeight也要加
    if (selectedTimes > 1)
    {
        this->m_iTotalEffectWeight *= selectedTimes;
        for (typename WeightInfoGroup<T>::iterator it = this->begin(); it < this->end(); ++it)
        {
            (*it)->m_iCurrentWeight += (*it)->m_iEffectiveWeight * (selectedTimes-1);
        }
    }
    selectedWeightInfo->m_iCurrentWeight -= this->m_iTotalEffectWeight;
...
template<class T>
void WeightInfoGroup<T>::beginSelect() 
{
    this->m_iNextSelectIdx = 0;
}

template<class T>
bool WeightInfoGroup<T>::getNext(T& outData)
{
    if (m_iNextSelectIdx < 0)
    {
        printf("Not call beginSelect? why it's < 0?? [%d]\n", m_iNextSelectIdx);
        m_iNextSelectIdx = 0;
    }

    // begin()和end()会变化
    if ((size_t)this->m_iNextSelectIdx >= this->size())
    {
        return false;
    }
    outData = this->operator[](m_iNextSelectIdx)->getData();
...
template<class T>
bool WeightInfoGroup<T>::endSelect(size_t selectedTimes)
{
    if (m_iNextSelectIdx <= 0 || m_iNextSelectIdx > (int)this->size())
    {
        fprintf(stderr, "nextSelectIdx out of [1, %u]:[%d]\n", (unsigned int)this->size(), m_iNextSelectIdx);
        return false;
    }
    updateSelectedCurrentWeight(this->operator[](m_iNextSelectIdx-1), selectedTimes);
    m_iNextSelectIdx = 0;
